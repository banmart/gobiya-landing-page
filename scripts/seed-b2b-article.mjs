/**
 * Supabase seed script: inserts/upserts the Automated B2B Sales Pipeline SEO article.
 * 
 * Run from the project root:
 *   node scripts/seed-b2b-article.mjs
 * 
...
const article = {
  ...
  image_url: '/images/article-b2b-pipeline-seo.webp',
  image_path: null,
  featured: true,
};
...
async function seed() {
  ...
  if (existing) {
    ...
    if (error) {
      ...
      console.error('   Note: If the "slug" or "featured" column does not exist yet, run the migration below first.');
      console.error('\n   Migration SQL:');
      console.error('   ALTER TABLE insights ADD COLUMN IF NOT EXISTS slug TEXT;');
      console.error('   ALTER TABLE insights ADD COLUMN IF NOT EXISTS featured BOOLEAN DEFAULT false;');
      process.exit(1);
    }
